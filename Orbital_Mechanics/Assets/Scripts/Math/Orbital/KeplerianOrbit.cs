...
namespace Sim.Math
{
    ...
    public class KeplerianOrbit
    {
        ...
        private void ChangeOrbitType(OrbitType type, Celestial centralBody)
        {
            switch (type)
            {
        ...
        public void UpdateAnomalies(float time) {
            var mat = orbit.UpdateAnomalies(elements, time);
            elements.meanAnomaly = mat.Item1;
            elements.anomaly = mat.Item2;
            elements.trueAnomaly = mat.Item3;
        }
        public (Vector3, Vector3) UpdateStateVectors() {
            return orbit.ConvertOrbitElementsToStateVectors(elements);
        }
