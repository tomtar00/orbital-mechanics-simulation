...
namespace Sim.Orbits
{
    public class HyperbolicOrbit : Orbit
    {
        ...
        public override double KeplerEquation(double H, double e, double M)
        {
            H = MathLib.Clamp(H, -700, 700);
            return M - e * MathLib.Sinh(H) + H; // M - e*sinh(H) + H = 0
        }
        public override double d_KeplerEquation(double H, double e)
        {
            H = MathLib.Clamp(H, -700, 700);
        ...
        {
            double theta = MathLib.Acos(-1.0 / elements.eccentricity) - 0.01;
